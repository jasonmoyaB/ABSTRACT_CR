/*
   SCRIPT COMPLETO DE BASE DE DATOS - ABSTRACT_CR
   Para ejecutar en SQL Server Local (LocalDB, Express, etc.)
   Incluye todas las tablas, campos adicionales y procedimientos almacenados
*/
...
IF NOT EXISTS (SELECT * FROM sys.databases WHERE name = 'Abstract_CR')
BEGIN
    CREATE DATABASE Abstract_CR;
END
GO
...
-- Agregar campo PuntosTotales a Usuarios si no existe
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.Usuarios') AND name = 'PuntosTotales')
BEGIN
    ALTER TABLE dbo.Usuarios
    ADD PuntosTotales INT NOT NULL CONSTRAINT DF_Usuarios_PuntosTotales DEFAULT (0);
END
GO

-- Agregar columna CorreoNorm (normalización de correo) si no existe
...
-- Agregar campos adicionales a MenusSemanales si no existen
IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.MenusSemanales') AND name = 'NombrePlatillo')
BEGIN
    ALTER TABLE dbo.MenusSemanales
    ADD NombrePlatillo NVARCHAR(200) NULL,
        DiaSemana NVARCHAR(20) NULL,
        Caracteristicas NVARCHAR(500) NULL,
        IngredientesPrincipales NVARCHAR(1000) NULL,
        TipChef NVARCHAR(500) NULL,
...
-- Tabla PuntosUsuarios (historial de puntos)
IF OBJECT_ID('dbo.PuntosUsuarios','U') IS NULL
BEGIN
    CREATE TABLE dbo.PuntosUsuarios (
        PuntosUsuarioId INT IDENTITY(1,1) CONSTRAINT PK_PuntosUsuarios PRIMARY KEY,
        UsuarioId INT NOT NULL,
        Puntos INT NOT NULL,
        Motivo NVARCHAR(250) NULL,
        FechaAsignacion DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME(),
        AsignadoPorId INT NULL,
        CONSTRAINT FK_PuntosUsuarios_Usuarios_UsuarioId FOREIGN KEY (UsuarioId) REFERENCES dbo.Usuarios(UsuarioID) ON DELETE CASCADE,
        CONSTRAINT FK_PuntosUsuarios_Usuarios_AsignadoPorId FOREIGN KEY (AsignadoPorId) REFERENCES dbo.Usuarios(UsuarioID) ON DELETE NO ACTION
    );
    CREATE INDEX IX_PuntosUsuarios_UsuarioId ON dbo.PuntosUsuarios(UsuarioId);
    CREATE INDEX IX_PuntosUsuarios_AsignadoPorId ON dbo.PuntosUsuarios(AsignadoPorId);
END
GO
...
-- Tabla PassResetTokens (para recuperación de contraseñas)
IF OBJECT_ID('dbo.PassResetTokens','U') IS NULL
BEGIN
    CREATE TABLE dbo.PassResetTokens (
        Id INT IDENTITY(1,1) CONSTRAINT PK_PassResetTokens PRIMARY KEY,
        UsuarioID INT NOT NULL,
        Token NVARCHAR(255) NOT NULL,
        FechaCreacion DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME(),
        FechaExpiracion DATETIME2 NOT NULL DEFAULT DATEADD(HOUR, 24, SYSUTCDATETIME()),
        Usado BIT NOT NULL DEFAULT 0,
        FechaUso DATETIME2 NULL,
        CONSTRAINT FK_PassResetTokens_Usuarios FOREIGN KEY (UsuarioID) REFERENCES dbo.Usuarios(UsuarioID) ON DELETE CASCADE
    );
    CREATE INDEX IX_PassResetTokens_UsuarioID ON dbo.PassResetTokens(UsuarioID);
    CREATE INDEX IX_PassResetTokens_Token ON dbo.PassResetTokens(Token);
END
GO

-- Agregar campos adicionales a PassResetTokens si la tabla ya existe
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'PassResetTokens')
BEGIN
    IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.PassResetTokens') AND name = 'FechaExpiracion')
    BEGIN
        ALTER TABLE dbo.PassResetTokens ADD FechaExpiracion DATETIME2 NOT NULL DEFAULT DATEADD(HOUR, 24, SYSUTCDATETIME());
    END
    
    IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.PassResetTokens') AND name = 'Usado')
    BEGIN
        ALTER TABLE dbo.PassResetTokens ADD Usado BIT NOT NULL DEFAULT 0;
    END
    
    IF NOT EXISTS (SELECT * FROM sys.columns WHERE object_id = OBJECT_ID('dbo.PassResetTokens') AND name = 'FechaUso')
    BEGIN
        ALTER TABLE dbo.PassResetTokens ADD FechaUso DATETIME2 NULL;
    END
    
    -- Actualizar longitud del Token si es necesario
    ...
    BEGIN
        ALTER TABLE dbo.PassResetTokens ALTER COLUMN Token NVARCHAR(255) NOT NULL;
    END
END
GO

-- Tabla MensajesInteraccion (para mensajes entre usuarios y chef)
IF OBJECT_ID('dbo.MensajesInteraccion','U') IS NULL
BEGIN
    CREATE TABLE dbo.MensajesInteraccion (
        MensajeInteraccionId INT IDENTITY(1,1) CONSTRAINT PK_MensajesInteraccion PRIMARY KEY,
        UsuarioId INT NOT NULL,
        RemitenteId INT NULL,
        EnviadoPorChef BIT NOT NULL DEFAULT 0,
        Contenido NVARCHAR(1000) NOT NULL,
        FechaEnvio DATETIME2 NOT NULL DEFAULT SYSUTCDATETIME(),
        Tipo INT NOT NULL DEFAULT 0,
        Leido BIT NOT NULL DEFAULT 0,
        CONSTRAINT FK_MensajesInteraccion_Usuarios_UsuarioId FOREIGN KEY (UsuarioId) REFERENCES dbo.Usuarios(UsuarioID) ON DELETE CASCADE,
        CONSTRAINT FK_MensajesInteraccion_Usuarios_RemitenteId FOREIGN KEY (RemitenteId) REFERENCES dbo.Usuarios(UsuarioID) ON DELETE NO ACTION
    );
    CREATE INDEX IX_MensajesInteraccion_UsuarioId ON dbo.MensajesInteraccion(UsuarioId);
    CREATE INDEX IX_MensajesInteraccion_RemitenteId ON dbo.MensajesInteraccion(RemitenteId);
END
GO
...
-- Único método Predeterminado por Usuario
...
-- Índices de soporte adicionales
